...
# Open the csv file as an object.  Opens the file.  R = read.  W = write.  X = execute 
with open(csvpath, 'r') as csvfile:

    # Pass in the csv file to the csv.reader() function
    # (with ',' as the delmiter/separator) and return the csvreader object
    csvreader = csv.reader(csvfile, delimiter=',')
#deliminiter = can have a comma separated file or csv or tab separated, deliminator states what is being separated.  Hypothetically can separate using any key on the computer

    # Read the header row first (skip this step if there is no header). READING
    ...
    # Read each row of data after the header.  For every row, 
